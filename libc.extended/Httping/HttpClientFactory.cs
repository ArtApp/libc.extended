...
namespace libc.extended.Httping {
    public class HttpClientFactory : IHttpClientFactory {
        ...
        private HttpResponseMessage callRetry(IFlurlClient client, string url,
            Action<IFlurlRequest> configure, out List<Exception> exceptions, HttpMethod method, HttpContent content = null,
            int retry = 5) {
        ...
        private HttpResponseMessage call(IFlurlClient client, string url,
            ...
            try {
                ...
                x(req);
                if (method == HttpMethod.Post) return req.PostAsync(content).Result;
                if (method == HttpMethod.Delete) return req.DeleteAsync().Result;
                if (method == HttpMethod.Head) return req.HeadAsync().Result;
                if (method == HttpMethod.Options) return req.OptionsAsync().Result;
                if (method == HttpMethod.Put) return req.PutAsync(content).Result;
                return req.GetAsync().Result;
            } catch (Exception ex) {
                exception = ex;
